...
class WorkoutRepositoryImpl(private val profileId: String) : WorkoutRepository {
    ...
    override suspend fun getAllWorkoutsForStats(): List<Workout> = withContext(Dispatchers.IO) {
        val client = OkHttpClient()
        val request = Request.Builder()
            .url("https://attsgwsxdlblbqxnboqx.supabase.co/rest/v1/workouts?profile_id=eq.$profileId&select=*")
        ...
        try {
            ...
            if (!response.isSuccessful) {
                println("Error fetching workouts: HTTP ${response.code}")
            ...
            println("Error fetching workouts: ${e.message}")
            e.printStackTrace()
            emptyList()
        }
    }
